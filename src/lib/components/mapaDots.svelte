<script>
	...
	let mapContainer;
	let cityInput = '';
	export let address = 'Spacerowa 125, 34-124 Klecza Dolna';
	...
	const updateCircleColors = (polygon) => {
		...
		circles.forEach((circle) => {
			...
			if (turf.booleanPointInPolygon([lngLat.lng, lngLat.lat], polygon)) {
				circle.getElement().style.backgroundColor = '#60cf5e';
			} else {
			...
			for (let i = 0; i < numRows; i++) {
				for (let j = 0; j < numColumns; j++) {
					...
					const circle = new Marker({
						draggable: false,
						element: createCircleElement('#60cf5e', adjustedCircleSize, adjustedCircleSize)
					})
						.setLngLat([lng, lat])
						.addTo(map);
